...
use crate::processors::common::condition_utils::{
    ConditionEvaluator, 
    ConditionOperation
};
use crate::config::{
...
impl ProcessorConfig for RuleConfig {
    fn from_stage_config(config: &StageConfig) -> Result<Self> {
        ...
        let error_strategy = extract_param::<ErrorStrategy>(&config.parameters, "error_strategy", default_error_strategy());
        
        Ok(Self { rules, error_strategy })
        ...
        for (i, rule) in self.rules.iter().enumerate() {
            ...
            if ConditionOperation::from_str(&rule.condition.operation).is_none() {
                return Err(anyhow!("Rule {} has unsupported operation: '{}'", i, rule.condition.operation));
            }
...
impl RuleConfig {
    fn validate_action(action: &Action, context: &str) -> Result<()> {
        match action {
            ...
            Action::CopyField { source_field, target_field } => {
                ...
                if source_field == target_field {
                    return Err(anyhow!("{}: CopyField source and target are the same", context));
                }
            }
            Action::RenameField { old_field, new_field } => {
                ...
                if old_field == new_field {
                    return Err(anyhow!("{}: RenameField old and new field are the same", context));
                }
            }
            Action::ComputeField { field_path, expression } => {
                if field_path.is_empty() {
            ...
            Action::KeepOnlyFields { field_paths } => {
                ...
                for field_path in field_paths {
                    if field_path.is_empty() {
                        return Err(anyhow!("{}: KeepOnlyFields contains empty field_path", context));
                    }
                }
            }
...
impl RuleProcessor {
    pub fn new(name: &str, config: StageConfig) -> Result<Box<dyn Processor>> {
        let rule_config = RuleConfig::from_stage_config(&config)?;
        ...
        let result = match action {
            ...
            Action::CopyField { source_field, target_field } => {
                ...
                if let Some(source_value) = FieldUtils::extract_field_value(payload, source_field) {
                    ...
                    let err = anyhow!("Source field '{}' not found for copy operation", source_field);
            ...
            Action::ComputeField { field_path, expression } => {
                debug!("Computing field '{}' with expression '{}'", field_path, expression);
                match self.evaluate_expression(payload, expression) {
                    Ok(result) => {
                        FieldUtils::set_field_value(payload, field_path, Value::Number(Number::from_f64(result).unwrap_or(Number::from(0))))
                    }
                    Err(e) => {
            ...
            Action::DropMessage => {
                ...
                Ok(()) // This will be handled at a higher level
            }
            Action::PassThrough => {
                debug!("Pass through action - no modifications");
                Ok(()) // No changes to payload
            }
            Action::KeepOnlyFields { .. } => {
                debug!("KeepOnlyFields action - handled in execute_actions");
                Ok(()) // This is handled separately in execute_actions method
            }
        };
    ...
    fn handle_action_error(&self, error: anyhow::Error, action: &Action) -> Result<()> {
        match self.config.error_strategy {
            ...
            ErrorStrategy::Skip => {
                ...
                Ok(()) // Skip this action, continue with others
            }
            ErrorStrategy::Abort => {
                error!("Aborting message processing due to action error: {}", error);
                Err(error) // Stop processing this message
            }
            ErrorStrategy::UseDefault => {
                warn!("Action {:?} failed: {} (using default behavior)", action, error);
                // For now, this is the same as Continue, but could be enhanced
                // to provide default values for specific action types
                Ok(())
            }
        }
    }
    ...
    fn add_payload_to_context(&self, value: &Value, prefix: &str, context: &mut evalexpr::HashMapContext) {
        use evalexpr::{Value as EvalValue, ContextWithMutableVariables};
    ...
    fn evaluate_expression(&self, payload: &Value, expression: &str) -> Result<f64> {
        ...
        for (func, namespaced_func) in &math_functions {
            ...
            if let Ok(regex) = regex::Regex::new(&pattern) {
                processed_expression = regex.replace_all(&processed_expression, *namespaced_func).to_string();
            }
        }
    ...
    fn process_message(&self, mut message: Message) -> Result<Option<Message>> {
        ...
        for rule in &self.config.rules {
            if self.evaluate_condition(&message.payload, &rule.condition) {
                ...
                debug!("Rule condition not matched, executing else_actions for message from {}", message.source);
    ...
    fn execute_actions(&self, payload: &mut Value, actions: &[Action]) -> Result<()> {
        ...
        for action in actions {
            if action.needs_pre_computation() {
                if let Action::ComputeField { field_path, expression } = action {
                    debug!("Pre-computing field '{}' with expression '{}'", field_path, expression);
                    match self.evaluate_expression(payload, expression) {
                        Ok(result) => {
                        ...
                        Err(e) => {
                            error!("Failed to pre-compute field '{}' with expression '{}': {}", field_path, expression, e);
        ...
        sorted_actions.sort_by(|a, b| a.priority().partial_cmp(&b.priority()).unwrap_or(std::cmp::Ordering::Equal));
            ...
            match action {
                Action::ComputeField { field_path, .. } => {
                    ...
                    if let Some(computed_value) = computed_values.get(field_path) {
                        debug!("Setting pre-computed field '{}' to {}", field_path, computed_value);
                        FieldUtils::set_field_value(payload, field_path, Value::Number(Number::from_f64(*computed_value).unwrap_or(Number::from(0))))?;
                    }
                }
                Action::KeepOnlyFields { field_paths } => {
                    debug!("Applying destructive reset - keeping only fields: {:?}", field_paths);
...
impl Processor for RuleProcessor {
    async fn init(&mut self) -> Result<()> {
        tracing::info!(
            "Rule processor '{}' initialised with {} rules",
            self.name, self.config.rules.len()
        );
        Ok(())
    }
